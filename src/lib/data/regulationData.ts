import {
  ...
  allStatusTypes,
  AttackPowerType,
} from "./attackPowerTypes";
import { Attribute } from "./attributes";
import { AttackElementCorrect, Weapon } from "./weapon";
...
export interface EncodedRegulationDataJson {
  ...
  readonly statusSpEffectParams: {
    readonly [spEffectParamId in number]?: Partial<
...
function evaluateCalcCorrectGraph(calcCorrectGraph: CalcCorrectGraph) {
  ...
  for (let i = 1; i < calcCorrectGraph.length; i++) {
    ...
    const maxAttributeValue =
      i === calcCorrectGraph.length - 1 ? 148 : stage.maxVal;
    ...
    for (
      ...
      if (!arr[attributeValue]) {
        let ratio =
          (attributeValue - prevStage.maxVal) /
          (stage.maxVal - prevStage.maxVal);
        ...
        arr[attributeValue] =
          prevStage.maxGrowVal +
          (stage.maxGrowVal - prevStage.maxGrowVal) * ratio;
      }
    }
  }
...
export function decodeRegulationData({
  ...
  const calcCorrectGraphsById = new Map(
    Object.entries(calcCorrectGraphs).map(
      ([calcCorrectGraphId, calcCorrectGraph]) => [
        +calcCorrectGraphId,
  ...
  const attackElementCorrectsById = new Map<number, AttackElementCorrect>(
    Object.entries(attackElementCorrects).map(
      ([attackElementCorrectId, attackElementCorrect]) => [
  ...
  return weapons.map(
    ({
      ...
      const attackElementCorrect = attackElementCorrectsById.get(
        attackElementCorrectId
      );
      ...
      const attack: Weapon["attack"] = reinforceParams.map((reinforceParam) => {
        ...
        unupgradedAttack.forEach(([attackPowerType, unupgradedAttackPower]) => {
          attackAtUpgradeLevel[attackPowerType] =
            ...
            (reinforceParam.attack[attackPowerType] ?? 0);
        });
        ...
        statusSpEffectParamIds?.forEach((spEffectParamId, i) => {
          if (spEffectParamId) {
            const statusSpEffectParam =
              statusSpEffectParams[spEffectParamId + (offsets[i] ?? 0)];
      ...
      const attributeScaling: Weapon["attributeScaling"] = reinforceParams.map(
        (reinforceParam) => {
          const attributeScalingAtUpgradeLevel: Weapon["attributeScaling"][number] =
            {};
          unupgradedAttributeScaling.forEach(
