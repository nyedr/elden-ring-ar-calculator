...
import { CharacterAttributes, getAttackAttributes } from "@/hooks/useCharacter";
...
interface WeaponsTableProps {
  characterAttributes: CharacterAttributes; // Only pass what's needed
  characterIsTwoHanding: boolean;
...
const WeaponsTable = memo(function WeaponsTable({
  ...
  const weaponsColumns = useMemo<ColumnDef<WeaponAttackResult>[]>(
    () => [
      {
        header: "General Info",
      ...
      <DataTable
        ...
        selectedItems={selectedWeapons.map((weapon) => {
          return getWeaponAttack({
            weapon,
            attributes: getAttackAttributes(characterAttributes),
            twoHanding: characterIsTwoHanding,
